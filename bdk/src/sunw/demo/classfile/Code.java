...
final class Code extends Attribute
{
	...
	void addExceptionTableEntry(short start, short stop, short handler,
			ClassConstant ct) {
		exceptions.addElement(new ExceptionTableEntry(start, stop, handler, ct));
	...
	void addOpInt(byte opCode, int op) {
		addOp4(opCode, (byte) ((op >>> 24) & 0xff), (byte) ((op >>> 16) & 0xff),
				(byte) ((op >>> 8) & 0xff), (byte) (op & 0xff));
	}
...
final class ExceptionTableEntry
{
	...
	ExceptionTableEntry(short start, short stop, short handler, ClassConstant eType)
	{
		super();
