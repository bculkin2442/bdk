...
public final class DelegatorClassFile
{
    ...
    private Hashtable<String, String> generatedMethods = new Hashtable<>();
    ...
    private void allocateFields() {
        ...
        for (int i = 1; i < targets.length; i++)
        {
            ...
            classFile.addFieldDesc(new FieldDesc(fieldName, fieldTypeString,
                    FieldDesc.ACC_PRIVATE, classFile, null));

            FieldConstant field
            = classFile.addFieldConstant(className, fieldName, fieldTypeString);
    ...
    private void generateConstructor() {
        Code code = new Code(classFile, (short) (targets.length + 2),
                (short) (targets.length + 4));
        ...
        MethodConstant mc = classFile.addMethodConstant(
                ClassFile.fullyQualifiedForm(superclassName), "<init>", "()V");
        ...
        classFile.addMethodDesc(new MethodDesc("<init>", typeString,
                MethodDesc.ACC_PUBLIC, classFile, code));
    }
    ...
    private void generateDelegateMethod(FieldConstant targetField, Method m) {
        ...
        Code code = new Code(classFile, (short) (params.length + 2),
                (short) (2 + (2 * params.length)));
        ...
        MethodConstant mc = classFile.addMethodConstant(
                m.getDeclaringClass().getName().replace('.', '/'), m.getName(),
                typeString);
        ...
        MethodDesc md = new MethodDesc(m.getName(), typeString, MethodDesc.ACC_PUBLIC,
                classFile, code);
