...
public class LocalQuote
{
    ...
    static Hashtable<String, QuoteEvent> getQuotes(QuoteServer server, Vector<String> symbols) {
        Hashtable<String, QuoteEvent> rv = new Hashtable<>();
        for (int i = 0; i < symbols.size(); i++)
        {
            String symbol = symbols.elementAt(i).toUpperCase();
            int iprice = (int) ((50.0 * Math.random())
                    + (10.0 * (1.0 + Math.sin(x++ * dx))));
            ...
            Date date = new Date();
            QuoteEvent event
            = new QuoteEvent(server, symbol, date, price, bid, ask, open, volume);
            rv.put(symbol, event);
        }
