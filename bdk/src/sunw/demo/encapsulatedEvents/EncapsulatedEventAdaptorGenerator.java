/*
 *
 * @(#) EncapsulatedEventAdaptorGenerator.java 1.4@(#)
 *
 ...
 * sunw.demo.encapsulatedEvents.EncapsulatedEvent and implements the specified
 * sub-interface of java.util.EventListener. Each listener method of the
 * sub-interface calls into sunw.demo.encapsulatedEvents.EncapsulatedEvent to
 * construct and EncapsulatedEvent and subsequently deliver that to the
 * objects listening to the adaptor.
 * </p>
 */
...
public final class EncapsulatedEventAdaptorGenerator extends ClassLoader
{
    ...
    private static EncapsulatedEventAdaptorGenerator generator
    = new EncapsulatedEventAdaptorGenerator();
     ...
     * adaptor subclass of EncapsulatedEventAdaptor to adapt to the specified
    ...
    protected Class<?> loadClass(String className, boolean resolve)
        ...
        if (c == null)
        { // not in cache
            if (isAdaptorClassName(className))
            { // is this an adaptor?
    ...
    /**
     * @return the name of the sub-interface we are adapting from the name of the
     *         adaptor class.
     */
    ...
    private Class<?> generateAdaptorClass(String className) {
        ...
        try
        { // generate the classfile into the stream
            new EncapsulatedEventAdaptorClassFile(className, baos);
