		...
		implements ActionListener, KeyListener, MouseListener
{
	 ...
	 *
	 */
	private static final long serialVersionUID = 5093380460781665092L;
	...
	public BridgeTester()
	{
		...
		{
				"TOTO", "s'en", "va", "en", "guerre"
		};
		setStringArray(myStringArray);
		float[] myFloatArray =
		{
				(float) 1.01, (float) 2.02, (float) 3.03, (float) 4.04, (float) 5.05
		};
	...
	public void setIntValue(int newValue) throws PropertyVetoException {
		...
		vetos.fireVetoableChange("intValue", new Integer(oldValue),
				new Integer(newValue));
		intValue = newValue;
		intField.setText(String.valueOf(intValue));
		changes.firePropertyChange("intValue", new Integer(oldValue),
				new Integer(newValue));
	}
	...
	public void setDoubleValue(double newValue) {
		...
		changes.firePropertyChange("doubleValue", new Double(oldValue),
				new Double(newValue));
	}
	...
	public void setShortValue(short newValue) {
		...
		changes.firePropertyChange("shortValue", new Short(oldValue),
				new Short(newValue));
	}
	...
	public void setFloatValue(float newValue) {
		...
		changes.firePropertyChange("floatValue", new Float(oldValue),
				new Float(newValue));
	}
	...
	public void setCharValue(char newValue) {
		...
		changes.firePropertyChange("charValue", new Character(oldValue),
	...
	public void setBooleanValue(boolean newValue) {
		...
		changes.firePropertyChange("booleanValue", new Boolean(oldValue),
	...
	private void fireEvent(int eventNumber) {
		...
		for (int i = 0; i < listenerObjects.size(); i++)
		{
			...
			{
			case 1:
				BridgeTesterEvent evt
						= new BridgeTesterEvent(this, stringValue, intValue);
				l.eventNumber1(evt);
				break;
			case 2:
	...
	public void keyReleased(KeyEvent e) {
		...
		if (source == doubleField)
		{
			...
			{
				double oldValue = doubleValue;
				doubleValue = new Double(doubleField.getText());
				changes.firePropertyChange("doubleValue", new Double(oldValue),
		...
		if (source == intField)
		{
			...
			try
			{
				int newValue = new Integer(intField.getText());
				vetos.fireVetoableChange("intValue", new Integer(oldValue),
						new Integer(newValue));
				intValue = newValue;
				changes.firePropertyChange("intValue", new Integer(oldValue),
		...
		if (source == shortField)
		{
			...
			{
				short oldValue = shortValue;
				shortValue = new Short(shortField.getText());
				changes.firePropertyChange("shortValue", new Short(oldValue),
		...
		if (source == floatField)
		{
			...
			{
				float oldValue = floatValue;
				floatValue = new Float(floatField.getText());
				changes.firePropertyChange("floatValue", new Float(oldValue),
		...
		if (source == byteField)
		{
			...
			{
				byte oldValue = byteValue;
				byteValue = new Byte(byteField.getText());
				changes.firePropertyChange("byteValue", new Byte(oldValue),
						new Byte(byteValue));
			} catch (java.lang.NumberFormatException ex)
			{
