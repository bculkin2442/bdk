...
public class AppletClassGenerator
{
     ...
     */
    void generateBeanInitialization(Wrapper beanWrapper, IndentedStream indentedStream) {
        BeanPropertyList pl = new BeanPropertyList(beanWrapper);

        Wrapper topBeanBoxWraper = beanBox.getTopWrapper();

        if (topBeanBoxWraper != null && (topBeanBoxWraper == beanWrapper
    ...
    private String generateJavaFile(String appletName, File dir) {
        ...
        {
            p.ip("InputStream is = this.getClass().getResourceAsStream(\"" + appletName
                    + "Data\");");
            p.pp("if (is == null) {");
            p.ip("System.err.println(\"Could not locate the Applet data at " + appletName
                    + "Data\");");
            p.pp("throw new Error(\"Could not locate the Applet data at " + appletName
                    + "Data\");");
            p.op("}");
        ...
        for (int i = 0; i < beanCount; i++)
        {
            ...
            p.pp("private " + bean.getClass().getName() + " " + uniqueName(bean) + ";");
        }
        ...
        for (int i = 0; i < beanCount; i++)
        {
            ...
            for (WrapperEventInfo element : wei)
            {
                if (element instanceof WrapperPropertyEventInfo)
                {
                ...
                } else
                {
                    ...
                    p.pp("private " + element.getAdaptorClassName() + " hookup" + count
                            + ";");
    ...
    private Rectangle getBeanBounds(Wrapper w) {
        ...
        if (bean instanceof Component)
        {
            ...
            return new Rectangle(wr.x + br.x, wr.y + br.y, br.width, br.height);
        } else
        {
            return null;
        }
    ...
    private void positionWithinField(Object bean, Rectangle bounds) {
        ...
        if (bounds != null)
        {
            p.pp("acquire(" + name + ", " + "new Rectangle(" + bounds.x + ", " + bounds.y
    ...
    private void generateAddConnections() {
        ...
        for (int i = 0; i < beanCount; i++)
        {
            ...
            for (WrapperEventInfo element : wei)
            {
                ...
                {
                    // This is a (bound) property hookup
                    WrapperPropertyEventInfo wpei = (WrapperPropertyEventInfo) element;
                    foundPropertyAdaptor = true;
                    ...
                    p.pp("\"" + wpei.getSetterName() + "\", " + wpei.getSetterTypes()
                    + ");");
                    p.o();
                } else
                {
                    ...
                    p.pp("hookup" + count + " = new " + element.getAdaptorClassName()
                    + "();");
                    p.pp("hookup" + count + ".setTarget("
                            + uniqueName(element.getTargetBean()) + ");");
                    p.pp(uniqueName(bean) + "."
                            + w.getAdderName(element.getEventSetName()) + "(" + "hookup"
                            + count + ");");
                }
                count += 1;
            }
    ...
    private void generateReconnections() {
        ...
        for (int i = 0; i < beanCount; i++)
        {
            ...
            for (WrapperEventInfo element : wei)
            {
                if ((element.getTargetBean() instanceof Externalizable) == false)
                {
                    count += 1;
    ...
    private void generateInitCode(String appletName) {
        ...
        for (int i = 0; i < beanCount; i++)
        {
            ...
            if (beanHasHiddenState(bean) == false)
            {
                p.pp(uniqueName(bean) + " = (" + bean.getClass().getName()
                        + ") Beans.instantiate(myLoader,\"" + bean.getClass().getName()
                        + "\");");
    ...
    protected boolean shouldSerializeHiddenStateBeans() {
        return (hiddenStateBeans != null && hiddenStateBeans.size() > 0) ? true : false;
    }
    ...
    protected void findBeansWithHiddenState() {
        for (int i = 0; i < beanCount; i++)
        {
            ...
            try
            {
                ...
                Boolean hiddenStateProperty = (Boolean) bd.getValue("hidden-state");
    ...
    private Vector<Object> hiddenStateBeans;
    private static Hashtable<Object, Comparable<?>> names = new Hashtable<>(); // unique names
...
class BeanProperty
{
    ...
    boolean isAccessibleToSourceCodeGeneration() {
        return (readMethod != null && writeMethod != null && propertyEditor != null
    ...
    String getJavaInitializationStatement(boolean use_this_identifier) {
        ...
        try
        {
            ...
            if (writeMethodThrowsCheckedExceptions())
            {
                ...
                String exceptionId = AppletClassGenerator.uniqueName(new Exception());
                s.append(" } catch (Exception " + exceptionId + ") { System.err.println("
                        + exceptionId + "); }");
            }
        } catch (Exception e)
        {
...
class BeanPropertyList
{
    BeanPropertyList(Wrapper beanWrapper)
    {
        ...
        try
        {
            ...
            {
                BeanProperty bp = new BeanProperty(bean, prop);
                beanProps.addElement(bp);

                if (isFromPrototype || (changedProperties != null
                        && changedProperties.contains(prop)))
                {
                    bp.setChanged();
                }
            }
    ...
    private void genInitCodeForBean(IndentedStream o, boolean useThisIdentifier) {
        ...
        for (Enumeration<BeanProperty> e = beanProps.elements(); e.hasMoreElements();)
        {
            BeanProperty bp = e.nextElement();
    ...
    private Vector<BeanProperty> beanProps;
}
