...
public class PropertySheet extends Frame
{
	 ...
	 *
	 */
	private static final long serialVersionUID = -668840238836526965L;
...
class PropertySheetPanel extends Panel
{
	 ...
	 *
	 */
	private static final long serialVersionUID = 3895347861599190025L;
	...
	synchronized void setTarget(Wrapper targ) {
		...
		for (int i = 0; i < properties.length; i++)
		{
			...
			try
			{
				...
				if (editor == null)
				{
					...
					String getterClass
							= properties[i].getReadMethod().getDeclaringClass().getName();
		...
		if (needPane)
		{
			...
			pane.setBounds(ins.left, ins.top, frameWidth - (ins.left + ins.right),
		...
		if (evt.getSource() instanceof PropertyEditor)
		{
			...
			for (int i = 0; i < editors.length; i++)
			{
				if (editors[i] == editor)
				{
					...
					{
						error("Unexpected exception while updating " + property.getName(),
								ex);
					}
