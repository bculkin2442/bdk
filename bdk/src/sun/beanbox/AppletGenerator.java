...
public class AppletGenerator
{
    ...
    public AppletGenerator(Frame ff, BeanBox box, File adir, String aname, String jname,
            File tdir)
    {
        frame = ff;
        beanBox = box;
    ...
    public static void generate(Frame frame, BeanBox bb, MakeAppDlg dialog,
            String appletDirName, String appletName, String jarName) {
        ...
        AppletGenerator g
        = new AppletGenerator(frame, bb, beandir, appletName, jarName, tmpdir);
    ...
    private void initializeJars() {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            String shortName = jarName.substring(jarName.lastIndexOf(File.separator) + 1);
            longJars.put(shortName, jarName);
        }
        String back[] = new String[longJars.size()];
        Enumeration<String> e = longJars.keys();
        for (int i = 0; e.hasMoreElements(); i++)
        {
            back[i] = e.nextElement();
        }
        shortJars = back;
    }
    ...
    private void initializeHookups() {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            {
                String s = element.getAdaptorClassName();
    ...
    private void writeAppletClass() throws IOException {
        ...
        Enumeration<String> e = longJars.elements();
        for (int i = 0; e.hasMoreElements(); i++)
        {
            jarNames[i] = e.nextElement();
        }
    ...
    private void writeInitializeData() throws IOException {
        ...
        {
            System.err
            .println("could not create output stream for serialized data; " + ex);
            System.exit(1);
        }
        int count = beanBox.getComponentCount();
        ...
        // Remove all listeners (targets and even the Wrapper itself) - if new event
        // model
        for (int i = 0; i < count; i++)
        {
            Wrapper w = (Wrapper) beanBox.getComponent(i);
    ...
    private void copyHookups() throws IOException {
        ...
        {
            // The class files
            String s = hookup.replace('.', File.separatorChar).concat(".class");
            File src = new File(s);
            File dst = new File(tmpDir, s);
            copyFile(src, dst);
            // The source files
            s = hookup.replace('.', File.separatorChar).concat(".java");
            src = new File(s);
    ...
    private void copyJars() throws IOException {
        ...
        for (String jar : jars)
        {
            File src = new File(getLongJar(jar));
            File dst = new File(appletDir, jar);
            copyFile(src, dst);
        }
    }
    ...
    private void writeJarFile() throws IOException {
        ...
        boolean jarAppletData = appletClassGen.shouldSerializeHiddenStateBeans();
        ...
        OutputStream out = new FileOutputStream(new File(appletDir, jarName + ".jar"));
     ...
     */
    private void writeMakefile(int type, String mname, boolean usesSerializedAppletData) {
        File f;
        ...
        try
        {
            IndentedStream p = new IndentedStream(
            ...
            {
                p.pn(" \\");
                p.pp0(hookup.replace('.', fsep).concat(".class"));
            }
            p.pp();
            p.op();
            ...
            if (type == unix)
            {
                ...
                p.pp("jar cfm $(JARFILE) manifest.tmp " + appletName
                        + ".class $(HOOKUPS) " + appletData);
                p.pp("@/bin/rm manifest.tmp");
                p.op();
            } else if (type == win32)
            {
                p.ip("jar cfm $(JARFILE) <<manifest.tmp " + appletName
                        + ".class $(HOOKUPS) " + appletData);
            ...
            if (type == unix)
            {
                p.ip("export CLASSPATH; CLASSPATH=." + psep + "$(JARPATH)" + psep
                        + "$(CLASSPATH); \\");
            } else if (type == win32)
            {
    ...
    private Hashtable<String, String> longJars; // original names, indexed by short names
