...
public class JarAccess
{
	 ...
	 */
	public static void create(OutputStream out, File baseDir, String beanFile,
	...
	public static JarEntrySource makeManifestEntry(String beanName) {
		...
		s.append("\n");
		return new JarEntrySource(MANIFEST, new StringBufferInputStream(s.toString()));
	}
	...
	public static void create(OutputStream out, String[] files) throws IOException {
		...
		{
			addEntry(zos, new JarEntrySource(new File(file)));
		}
		zos.close();
	}
	 ...
	 */
	public static void create(OutputStream out, JarEntrySource[] entries)
			throws IOException {
		ZipOutputStream zos = new ZipOutputStream(out);
		for (JarEntrySource entrie : entries)
		{
			addEntry(zos, entrie);
		}
		zos.close();
	 ...
	 */
	private static void extract(InputStream in, String files[]) throws IOException {
