...
implements Serializable, MouseListener, MouseMotionListener
{
    ...
    private Vector<WrapperPropertyEventInfo> getWPEIfromWET(Vector<WrapperEventTarget> wets) {
        Vector<WrapperPropertyEventInfo> back = new Vector<>();
        // we only care about properties, not method hookups
        for (Enumeration<WrapperEventTarget> e = wets.elements(); e.hasMoreElements();)
        {
            WrapperEventTarget wet = e.nextElement();
            if (wet.targetListener instanceof PropertyHookup)
            {

                PropertyHookup h = (PropertyHookup) wet.targetListener;
                Hashtable<String, Vector<PropertyHookupTarget>> table = h.getTargetsByProperty();
                for (Enumeration<String> keys = table.keys(); keys.hasMoreElements();)
                {
                    String propertyName = keys.nextElement();
                    Vector<PropertyHookupTarget> targets = table.get(propertyName);

                    for (Enumeration<PropertyHookupTarget> ee = targets.elements(); ee.hasMoreElements();)
                    {
                        Object t = ee.nextElement();

                        back.addElement(new WrapperPropertyEventInfo(h.getTargetObject(t),
                                propertyName, h.getSetterMethod(t)));
                    }
                }
            }
        }
    ...
    public String getAdderName(String eventSetName) {
        EventSetDescriptor esd = esdMap.get(eventSetName);
        Method adder;
    ...
    public WrapperEventInfo[] getEventHookupInfo() {
        ...
        int i = 0;
        Enumeration<WrapperEventTarget> e = eventTargets.elements();
        while (e.hasMoreElements())
        {
            WrapperEventTarget et = e.nextElement();
            ...
            } else
            {
                back[i] = new WrapperEventInfo(et.targetBean,
                        et.targetListener.getClass().getName(), et.eventSetName);
                i += 1;
            }
        }
        Enumeration<WrapperEventInfo> e2 = propertyTargets.elements();
        while (e.hasMoreElements())
        {
            back[i] = e2.nextElement();
            i += 1;
        }
        return back;
    }
    ...
    synchronized void addPropertyTarget(String propertyName, Object targetObject,
            Method setter) {
        propertyTargets.addElement(
                new WrapperPropertyEventInfo(targetObject, propertyName, setter));
    }

    // Add a hookup. All property bound hookups are represented by (at most) one
    // hookup
    synchronized void addEventTarget(String eventSetName, Wrapper targetWrapper,
            Object listener) {
        ...
        {
            System.err.println(
                    "Internal error: Wrapper.addEventTarget missing add/remote listener");
            System.err.println("        eventSetName = " + eventSetName);
            System.err.println("        bean = " + bean);
            return;
        }
        ...
        {
            System.err.println(
                    "Wrapper: adding event listener for " + eventSetName + " failed:");
    ...
    void removeListeners() {
        Enumeration<WrapperEventTarget> enm = eventTargets.elements();
        while (enm.hasMoreElements())
        {
            WrapperEventTarget et = enm.nextElement();
            EventSetDescriptor esd = esdMap.get(et.eventSetName);
            Method remover = esd.getRemoveListenerMethod();
            try
            {
                Object args[] =
    ...
    void attachListeners() {
        ...
        while (enm.hasMoreElements())
        {
            ...
            {
                System.err.println("Wrapper: adding event listener for " + et.eventSetName
                        + " failed:");
                System.err.println("    bean = " + bean);
                System.err.println("    " + ex.getTargetException());
                ex.getTargetException().printStackTrace();
    ...
    static void showInvisibleBeans(boolean show) {
        ...
        {
            invisibleWrappers.elementAt(i).setVisible(show);
        }
    }
    ...
    public boolean handleEvent(Event evt) {
        ...
        } else if (evt.id == Event.MOUSE_UP)
        {
            if (sawMouseDown)
            {
                me = new MouseEvent(source, MouseEvent.MOUSE_CLICKED, 0, 0, x, y, 0,
                        false);
                mouseClicked(me);
            }
            sawMouseDown = false;
    ...
    // ----------------------------------------------------------------------
    private static Hashtable<Class<?>, Boolean> eventModelCache = new Hashtable<>();

    // Check if our child needs to use the old AWT event mnodel.

    private synchronized boolean useNewEventModel() {
        // Check our cache first.
        Boolean b = eventModelCache.get(child.getClass());
        if (b != null)
        {
            return b;
        }
    ...
    private static Vector<Wrapper> invisibleWrappers = new Vector();
    private transient boolean sawMouseDown;

    // Table that maps events set names to EventDescriptors.
    private transient Hashtable<String, EventSetDescriptor> esdMap;
    // List of WrapperEventTargets that we fire events at.
    private Vector<WrapperEventTarget> eventTargets;
