...
class ToolBox extends Frame
{
     ...
     *
     */
    private static final long serialVersionUID = 7273434526261587168L;
...
class ToolBoxScrollPane extends ScrollPane
{
     ...
     *
     */
    private static final long serialVersionUID = -2619955628316933347L;
...
class ToolBoxPanel extends Panel implements Runnable, MouseListener
{
     ...
     *
     */
    private static final long serialVersionUID = 76623215689163447L;
    ...
    void addWithUniqueName(Vector<String> v, String s) {
        if (!v.contains(s))
        {
            debug("adding " + s);
            v.addElement(s);
    ...
    synchronized void addBeanClass(String beanClassName) {
        ...
        {
            System.err.println("Toolbox: couldn't instantiate " + beanClassName);
            return;
        }
        BeanInfo bi;
        try
        ...
        {
            System.err.println("Toolbox: couldn't find BeanInfo for " + beanClassName);
            ex.printStackTrace();
            return;
        }
    ...
    public Dimension getPreferredSize() {
        ...
        {
            return new Dimension(145,
                    rowHeight * (beanLabels.size() + 1) + frame.getInsets().bottom);
        } else
        {
    ...
    public void paint(Graphics g) {
        ...
        {
            String name = beanLabels.elementAt(i);
            g.drawString(name, sidePad + 21, topPad + (rowHeight * i) + rowHeight - 4);
            Image img = beanIcons.elementAt(i);
            if (img != null)
            {
                boolean status = g.drawImage(img, sidePad + 2,
                        topPad + (rowHeight * i) + 2, 16, 16, this);
            }
        }
    }
     ...
     */
    public Vector<JarInfo> getLoadedJarInfo() {
        return beanJars;
    }

    private static Vector<String> getJarNames() {
        ...
        {
            result.addElement(jars.getPath() + File.separatorChar + name);
        }
        names = jars.list(new FileExtension(".JAR"));
        for (String name : names)
        {
        ...
        for (int i = result.size() - 2; i >= 0; i--)
        {
            for (int j = 0; j <= i; j++)
            {
                String s1 = result.elementAt(j);
                String s2 = result.elementAt(j + 1);
                if (s1.compareTo(s2) > 0)
                {
                    String tmp = result.elementAt(j);
    ...
    public void mousePressed(MouseEvent evt) {
        ...
        boolean fromPrototypeInfo = false;
        String beanName = beanNames.elementAt(row);
        String beanLabel = beanLabels.elementAt(row);
        if (beanName.equals("sun.beanbox.BeanBox"))
        {
            bean = new BeanBox();
        } else
        {
            JarInfo ji = beanJars.elementAt(row);
            try
            {
    ...
    Vector<String> beanLabels = new Vector<>();
    Vector<String> beanNames = new Vector<>();
    Vector<Image> beanIcons = new Vector<>();
    Vector<JarInfo> beanJars = new Vector<>();
