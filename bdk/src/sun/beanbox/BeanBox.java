...
public class BeanBox extends Panel implements Serializable, Runnable, MouseListener,
MouseMotionListener, BeanContextProxy
{
    ...
    public BeanBox()
    {
        ...
        bcss.addService(javax.infobus.InfoBus.class, new BeanContextServiceProvider()
        {

            @Override
            public Object getService(BeanContextServices bcs, Object requestor,
                    Class serviceClass, Object serviceSelector) {
                if (serviceSelector != null && ((serviceSelector instanceof String
                        && (String) serviceSelector != ibn) || true))
                    return null;
                return ib;
            }
            ...
            public Iterator getCurrentServiceSelectors(BeanContextServices bcs,
                    Class serviceClass) {
                return Arrays.asList(ibn).iterator();
            }
        });
    ...
    public synchronized void updateMenuBar(MenuBar bar) {
        ...
        try
        {
            ...
            if (esd.length > 0)
            {
                ...
                eventMap = new Hashtable<Menu, EventSetDescriptor>();
                methodMap = new Hashtable<MenuItem, Method>();
                // We create a separate menu for each event listener interface,
                // with a menu item for each event handler method.
                for (EventSetDescriptor element : esd)
                {
                    String dname = element.getDisplayName();
                    Menu methodMenu = new Menu(dname);
                    eventMap.put(methodMenu, element);
                    Method methods[] = element.getListenerMethods();
                    for (Method method : methods)
                    {
                        String mname = method.getName();
                        MenuItem mi = new MenuItem(mname);
                        addMenuItem(frame, methodMenu, mi);
                        methodMap.put(mi, method);
                    }
                    eventMenu.add(methodMenu);
                    if (element.getName().equals("propertyChange"))
                    {
                        bindable = true;
                    }
                }
    ...
    private void writeContents(java.io.ObjectOutputStream s) throws java.io.IOException {
        int count = getComponentCount();
    ...
    private void print() {
        ...
        PrintJob pj = Toolkit.getDefaultToolkit().getPrintJob(ourFrame, "Printing Test",
                (Properties) null);
    ...
    private void loadJar() {
        FileDialog fd
        = new FileDialog(getFrame(), "Load beans from JAR File", FileDialog.LOAD);
        // the setDirectory() is not needed, except for a bug under Solaris...
        fd.setDirectory(System.getProperty("user.dir"));
        fd.setFile(defaultStoreFile);
        fd.show();
    ...
    void doBind() {
        ...
        dialog = new PropertyNameDialog(getFrame(), target, "Select target property",
    ...
    public void doInsert(Object bean, String beanLabel, String beanName,
        ...
        if (bean instanceof DesignMode)
        {
            ...
            boolean bcdmode = ((DesignMode) getBeanContextProxy()).isDesignTime();
    ...
    transient Hashtable<MenuItem, Method> methodMap; // maps MenuItems to Methods
    transient Hashtable<Menu, EventSetDescriptor> eventMap; // maps Menus to EventSetDescriptors
    transient Class<?> customizerClass;

    private static Hashtable<Object, String> serNames = new Hashtable<>();
