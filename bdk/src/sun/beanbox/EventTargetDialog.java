...
public class EventTargetDialog extends Dialog implements Runnable, ActionListener
{
	 ...
	 *
	 */
	private static final long serialVersionUID = -4156509864182797939L;
			...
			EventSetDescriptor esd, Method listenerMethod)
	{
		...
		{
			methods[i + matchMethods.size()]
					= (MethodDescriptor) zeroArgMethods.elementAt(i);
		}

		if (count == 0)
		{
			new ErrorDialog(frame,
					"No suitable target method on\n" + target.getClass().getName());
			return;
		}
	...
	public void run() {
		...
		HookupManager.hookup(esd, listenerMethod, sourceWrapper, targetWrapper,
				methods[index].getMethod());
