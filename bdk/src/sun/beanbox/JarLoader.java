...
public class JarLoader
{
    ...
    private JarInfo createJarInfo(Vector classList, Vector serList, Manifest mf) {
        ...
        Hashtable<String, MessageHeader> headersTable = new Hashtable<>();
        if (mf == null)
        {
        ...
        {
            beans = new Hashtable<>();
            for (Enumeration<MessageHeader> entries = mf.entries(); entries.hasMoreElements();)
            {
                MessageHeader mh = entries.nextElement();
        ...
        for (keys = beans.keys(), i = 0; keys.hasMoreElements(); i++)
        {
            ...
            beanNames[i] = key;
            fromPrototype[i] = (beans.get(key));
            headers[i] = headersTable.get(key);
        }
